	...
	for(unsigned zone_index=0; zone_index<nzones; ++zone_index) {
		...
		if(zone_index) {
			...
			inclination_evol[zone_index-1]=zone.inclination_evolution(
					zone_to_zone_transform(__body1.zone(0), zone,
										   reference_torque),
					torque);
		...
		double *periapsis_age_derivs, double *angmom_age_derivs) const
{
	...
	Eigen::Vector3d ref_torque=__body1.nontidal_torque(0),
					...
					dref_torque_dage=__body1.nontidal_torque(0,
							Dissipation::AGE),
					zero3d(0,0,0);
	for(unsigned deriv_zone_ind=0; deriv_zone_ind<nzones; ++deriv_zone_ind) {
	...
	for(unsigned zone_ind=1; zone_ind<nzones; ++zone_ind) {
		...
		Eigen::Vector3d 
			zone_ref_torque=zone_to_zone_transform(ref_zone, zone,
												   ref_torque),
			zone_torque=__body1.nontidal_torque(zone_ind),
			ref_torque_age_deriv=zone_to_zone_transform(
					ref_zone, zone, dref_torque_dage),
			zone_torque_age_deriv=__body1.nontidal_torque(
					zone_ind, Dissipation::AGE, 0);
		...
		for(unsigned quantity_ind=0; quantity_ind<nparams; ++quantity_ind) {
			...
			if(quantity_zone==zone_ind) {
				...
				inclination_dest=zone.inclination_evolution(
						ref_torque_deriv, zone_torque_deriv);
				periapsis_dest=zone.periapsis_evolution(
						ref_torque_deriv, zone_torque_deriv);
			}
	...
	if(std::isnan(orbit_energy_gain_deriv))
		...
		return factor*(	
					   ...
					   +
							 ...
							 +
							 orbit_angmom_gain/__semimajor)/__orbital_angmom
					  );
...
void BinarySystem::above_lock_problem_deriv_correction(
		Dissipation::Derivative deriv, bool body1_deriv,
		Eigen::MatrixXd &matrix, Eigen::VectorXd &rhs) const
{
		...
		if(deriv==Dissipation::SPIN_FREQUENCY 
			...
			matrix(deriv_zone_index, deriv_zone_index)-=
				(deriv_body.tidal_torque(0, true)[2]
				 -
				 deriv_body.tidal_torque(0, false)[2])*coef;
			rhs(deriv_zone_index)-=deriv_body.tidal_torque(0, false)[2]*coef;
		} else if(deriv==Dissipation::MOMENT_OF_INERTIA) {
			rhs(deriv_zone_index)-=deriv_zone.moment_of_inertia(1)
								   /std::pow(deriv_zone.moment_of_inertia(),
										     2);
		}
	}
}
		...
		Dissipation::Derivative deriv, bool body1_deriv)
{
	...
	for(std::list<unsigned>::const_iterator zi=__locked_zones.begin();
			zi!=__locked_zones.end(); ++zi) {
		...
		double *evolution_rates) const
{
	...
	double reference_periapsis_rate=NaN;
	for(unsigned zone_index=0; zone_index<num_total_zones; ++zone_index) {
		...
		Eigen::Vector3d 
			zone_orbit_torque=(zone_index
							   ? zone_to_zone_transform(reference_zone, zone,
				 					  			 	    global_orbit_torque)
							   : global_orbit_torque),
		...
		unsigned offset) const
{
	...
	orbit_rate_deriv[0]+=(body.*func)(Dissipation::SEMIMAJOR, 0,
									  Eigen::VectorXd());
	orbit_rate_deriv[1]+=(body.*func)(Dissipation::ECCENTRICITY, 0,
									  Eigen::VectorXd());
	orbit_rate_deriv[num_param]+=(body.*func)(Dissipation::AGE, 0,
											  Eigen::VectorXd())
								 +
								 (body.*func)(Dissipation::RADIUS, 0,
										 	  Eigen::VectorXd())
								 *body.radius(1);
	unsigned locked_zone_count=(offset ? __body1.number_locked_zones() : 0);
	for(unsigned zone_ind=0; zone_ind<body.number_zones(); ++zone_ind) {
		orbit_rate_deriv[zone_ind+2+offset]=
			(body.*func)(Dissipation::INCLINATION, zone_ind,
				__above_lock_fractions_inclination_deriv[zone_ind+offset]);
		if(zone_ind+offset>0)
			orbit_rate_deriv[zone_ind+1+num_zones+offset]=
				(body.*func)(Dissipation::PERIAPSIS, zone_ind, 
		...
		if(zone.locked()) ++locked_zone_count;
			...
			(body.*func)(Dissipation::SPIN_ANGMOM, zone_ind, 
				 __above_lock_fractions_angmom_deriv[zone_ind+offset]);
		orbit_rate_deriv[num_param]+=
			(body.*func)(Dissipation::MOMENT_OF_INERTIA, zone_ind,
...
{
	orbit_energy_gain_deriv[0]=orbit_energy_gain_deriv[1]=
	...
	add_body_rate_deriv(__body2, &DissipatingBody::tidal_orbit_energy_gain,
		...
		std::valarray<double> &orbit_angmom_gain_deriv) const
{
	...
	add_body_rate_deriv(__body1, &DissipatingBody::tidal_orbit_torque, 
			body1_orbit_torque_deriv, 0);
...
void BinarySystem::semimajor_jacobian(
		...
		bool a6p5, double *param_derivs, double &age_deriv) const
{
...
void BinarySystem::angle_evolution_age_deriv(DissipatingBody &body,
		unsigned zone_ind, double sin_inc, double cos_inc,
		unsigned locked_zone_ind, double &inclination, double &periapsis)
	const
{
	...
	unsigned body_zone_ind=0, num_zones=number_zones();
...
void BinarySystem::inclination_evolution_zone_derivs(
		...
		unsigned locked_zone_ind, double *result) const
{
#ifdef DEBUG
	assert(deriv==Dissipation::INCLINATION ||
			deriv==Dissipation::PERIAPSIS ||
...
void BinarySystem::spin_angmom_evolution_zone_derivs(
		...
		unsigned locked_zone_ind, double *result) const
{
	unsigned global_zone_ind=zone_ind, num_zones=number_zones();
	if(&body==&__body2) global_zone_ind+=__body1.number_zones();
	double above_frac=
		__above_lock_fractions[Dissipation::NO_DERIV][locked_zone_ind];
	...
	const
{
	...
	eccentricity_jacobian(orbit_energy_gain_deriv, orbit_angmom_gain_deriv,
			num_locked_zones==0, param_derivs+num_param, age_derivs[1]);
	unsigned locked_zone_ind=0;
	DissipatingBody *body=&__body1;
	unsigned body_zone_ind=0;
	...
	for(unsigned zone_ind=0; zone_ind<num_zones; ++zone_ind) {
		...
		angle_evolution_age_deriv(*body, zone_ind, sin_inc, cos_inc,
								  locked_zone_ind, age_derivs[2+zone_ind],
								  periapsis_age_deriv);

		double *periapsis_row=
			(zone_ind==0 ? &reference_periapsis_param_deriv[0]
			 			 : param_derivs+(1+zone_ind+num_zones)*num_param);

		angle_evolution_orbit_deriv(Dissipation::SEMIMAJOR, dangmom_da,
...
EvolModeType BinarySystem::fill_orbit(std::valarray<double> &orbit) const
{
	...
	else if(__evolution_mode==BINARY) fill_binary_orbit(orbit);
...
virtual void BinarySystem::reached_critical_age(double age)
{
    __body1.reached_critical_age(age);
    __body2.reached_critical_age(age);
}

virtual double BinarySystem::next_stop_age() const
{
