...
void CommandLineOptions::define_options()
{
	...
	option_help.str("");
	option_help << "The eccentricity at which the planet first "
			"appears in AU. In --input-columns identified by '"
			<< __input_column_names[InCol::E_FORMATION] << "'. Default: "
			<< __defaults[InCol::E_FORMATION] << ".";
	__direct_value_options[InCol::E_FORMATION]=arg_dbl0("e",
			"init-eccentricity", "<double>", cstr_copy(option_help));
	...
	__start_locked=arg_lit0(NULL, "use-YREC", "Whether to use the YREC "
		   "stellar evolution tracks instead of the MESA ones. The MESA "
           "tracks are used by default.");
...
void output_solution(const OrbitSolver &solver, const BinarySystem &system,
		const EvolvingStar &star, const std::string &filename,
...
void calculate_evolution(const std::vector<double> &real_parameters,
		...
		bool need_orbit)
{
	EvolvingStar star(real_parameters[InCol::MSTAR],
