...
logfile = open('./explorationpart/SurvivingAntiAligned/survivingantialignedgraphlogs.txt', 'w', 0)
...
def buildGraphString(M, m, period, Qin, Qout, Rotation):
    string = 'SurvivingAntiAlignedM_' + str(M) + 'm_' + str(m) + 'period_' + str(period)  + 'lgQin_' + str(Qin)  + \
...
def buildFractionGraph(M, m, period, Qin, Qout, Rotation):
    ...
    fig = plt.figure()
    plt.title(r'Fraction of Surviving Anti-Aligned Planets')
    ...
    axs.set_ylim([0, 1.0])
    sns.despine()
    plt.savefig('./explorationpart/SurvivingAntiAligned/' + buildGraphString(M, m, period, Qin, Qout, Rotation))
    plt.close(fig)
