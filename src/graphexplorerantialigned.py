...
logfile = open('./explorationpart/AntiAligned/antialignedgraphlogs.txt', 'w', 0)
...
def buildGraphString(M, m, period, Qin, Qout, Rotation):
    string = 'AntiAlignedAllM_' + str(M) + 'm_' + str(m) + 'period_' + str(period)  + 'lgQin_' + str(Qin)  + \
...
def buildFractionGraph(M, m, period, Qin, Qout, Rotation):
    ...
    for i in range(len(ageslist)):
        ...
        for evol in Evolutions:  
            ...
            elif obliquityAtAge >= np.radians(170):
    ...
    fig = plt.figure()
    plt.title(r'Fraction of Anti-Aligned Planets')
    ...
    axs.set_ylim([0, 1.0])
    sns.despine()
    plt.savefig('./explorationpart/AntiAligned/' + buildGraphString(M, m, period, Qin, Qout, Rotation))
    plt.close(fig)
