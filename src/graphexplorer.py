...
logfile = open('./explorationpart/AlignedPlanets/alignedgraphlogs.txt', 'w', 0)
...
def buildGraphString(M, m, period, Qin, Qout, Rotation):
    string = 'AlignedAllM_' + str(M) + 'm_' + str(m) + 'period_' + str(period)  + 'lgQin_' + str(Qin)  + \
...
def buildFractionGraph(M, m, period, Qin, Qout, Rotation):
    ...
    fig = plt.figure()
    plt.title(r'Fraction of Aligned Planets')
    ...
    axs.set_ylim([0, 1.0])
    sns.despine()
    plt.savefig('./explorationpart/AlignedPlanets/' + buildGraphString(M, m, period, Qin, Qout, Rotation))
    plt.close(fig)
