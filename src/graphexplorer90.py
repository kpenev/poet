...
logfile = open('./explorationpart/SurvivingSidePlanets/90graphlogs.txt', 'w', 0)
...
def buildGraphString(M, m, period, Qin, Qout, Rotation):
    string = 'Surviving90M_' + str(M) + 'm_' + str(m) + 'period_' + str(period)  + 'lgQin_' + str(Qin)  + \
...
def buildFractionGraph(M, m, period, Qin, Qout, Rotation):
    ...
    for i in range(len(ageslist)):
        ...
        for evol in Evolutions:  
            ...
            elif obliquityAtAge <= 100 and obliquityAtAge >= np.radians(80):
                alignedCount = alignedCount + 1
            
            
        fractionAtAges[i] = 1.0 * alignedCount/(len(Evolutions))
    
    fig = plt.figure()
    plt.title(r'Fraction of Surviving $\Theta = \frac{\pi}{2}$ Planets')
    ...
    axs.set_ylim([0, 1.0])
    sns.despine()
    plt.savefig('./explorationpart/SurvivingSidePlanets/' + buildGraphString(M, m, period, Qin, Qout, Rotation))
    plt.close(fig)
